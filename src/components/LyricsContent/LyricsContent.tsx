...
const LyricsContent: React.FC<LyricsContentProps> = ({
  ...
  const onSeek = usePlayerStore(state => state.handleSeek)
  ...
  useEffect(() => {
    ...
    if (autoScrollEndTimeoutRef.current) clearTimeout(autoScrollEndTimeoutRef.current)

    // Принудительно сбрасываем скролл контейнера
  ...
  useEffect(() => {
    ...
    const fetchLyrics = () => {
      ...
      const isSearch = !duration || duration === 0;
      const url = isSearch
      ...
      fetch(url)
        ...
        .catch((e) => {
          ...
          if (e.message && e.message.includes("Текст не найден для этого трека")) {
            // If we used search and didn't find it, we can mark as not found immediately
            setNotFound(true)
            notFoundTimeout.current = setTimeout(() => setNotFound(false), 3000)
          } else if (e.message && e.message.includes("Failed to find specified track")) {
            // Let's retry only if we tried to do exact match
            if (!isSearch) {
            ...
            syncedLines.map((line, index) => {
              ...
              return (
                <motion.div
                  ...
                  transition={{ scale: { type: "spring", stiffness: 300, damping: 15 }, borderRadius: { type: "spring", stiffness: 300, damping: 20 }, opacity: { duration: 0.3 }, filter: { duration: 0.2 } }}
                  ...
                  style={{
                    ...
                    padding: "8px",
                    marginBottom: "4px",
                    background: isCurrent ? "var(--dominant-color)" : "var(--dominant-color-transparent)",
