...
export default function AdminPlaylistsPage() {
  ...
  const onCoverUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      formData.append("trackId", editingPlaylist.id); 

      const res = await fetch("/api/admin/tracks/cover?type=playlist", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="space-y-8">
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="bg-[#0c0c0c] border-zinc-800 text-zinc-100 max-w-xl rounded-2xl max-h-[95vh] overflow-y-auto custom-scrollbar">
          <DialogHeader>
          ...
          <Tabs defaultValue="info" className="w-full">
            <TabsList className="mx-6 bg-zinc-900/50 border border-zinc-800 p-1 mb-2 rounded-lg h-9">
              <TabsTrigger value="info" className="data-[state=active]:bg-zinc-800 data-[state=active]:text-zinc-100 rounded-md px-4 font-bold uppercase tracking-widest text-[9px]">Информация</TabsTrigger>
              <TabsTrigger value="tracks" disabled={!editingPlaylist?.id} className="data-[state=active]:bg-zinc-800 data-[state=active]:text-zinc-100 rounded-md px-4 font-bold uppercase tracking-widest text-[9px]">Треки</TabsTrigger>
            </TabsList>

            <TabsContent value="info" className="p-6 pt-2 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
            ...
            <TabsContent value="tracks" className="p-6 pt-2 max-h-[70vh] overflow-y-auto custom-scrollbar">
          ...
          <DialogFooter className="gap-2">
            ...
            <Button className="bg-zinc-100 text-zinc-900 font-semibold hover:bg-white gap-2" onClick={onSave} disabled={isSaving}>
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent className="bg-[#0c0c0c] border-zinc-800 text-zinc-100 max-w-md rounded-2xl">
          ...
          <DialogFooter className="gap-2">
            <Button variant="outline" className="border-zinc-800 text-zinc-400 hover:text-zinc-100 hover:bg-zinc-900 flex-1" onClick={() => setIsDeleteOpen(false)} disabled={isDeleting}>
              Закрыть
            </Button>
            <Button className="bg-red-600 text-white font-semibold hover:bg-red-500 flex-1 gap-2" onClick={onDelete} disabled={isDeleting}>
